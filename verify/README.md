...
`verify.ts` runs this plugin's title-rendering path against a fixture `$HOME`
seeded with known cache-file contents. The `window-title-text` check is the
hard gate for rendered status tokens (`CC5`, `CCW`, `CCF`, `CX5`, `CXW` — and
never the retired `CCS`).

Unit tests exercise individual functions in isolation; this harness catches
what they miss: the title rendering chain (`engine.ts` → `display.ts`), cache
parsing through the real file-system path
...
3. **Restart long-running surfaces** — daemons keep running old code until
   restarted. For this plugin: invoke the `stop-title-daemon` /
   `start-title-daemon` actions. Confirm the restarted process's cwd is the
   linked tree (`lsof -p <pid> | awk '$4=="cwd"'`).
4. **Human confirms every user-visible surface** — the release judgment is the
   ...
   plugin: the outer window/tab title, cross-checked against the provider's
   usage page.
